...
class Common extends MX_Controller
{
	...
	public function audit_logs($data){
		$this->load->model('audit/audit_model');
		return $this->audit_model->insert($data);
	}

	public function get_slspn($data){
		$this->load->model('manage_users/users_model');
		$this->load->model('slspn/slspn_model');
        if(!isset($data['slspn_code']) OR $data['slspn_code'] == ""){
            if(in_array($this->session->userdata('user_role')['name'], ['K12_MA','DISTRIBUTOR_MA','LMD_MA','LSG_MA','TMD_MA','TMD_LMD_MA'])){
                $data['sales_person_code'] = $this->session->userdata('profile')['sales_person_code'];
            }
            if(in_array($this->session->userdata('user_role')['name'],["K12_RSM","LMD_RSM","TMD_RSM","LSG_RSM","TMD_LMD_RSM","DISTRIBUTOR_RSM"])){
				$data['sales_person_code'] = $this->slspn_model->slspn(["filters"=>[
					"rsm_bu" => $this->session->userdata('profile')['slspn_bu'],
					"rsm_code" => $this->session->userdata('profile')['sales_person_code'],
				]]);
				$data['sales_person_code'] = array_column($data['sales_person_code'],"ma_code");
            }
            if(in_array($this->session->userdata('user_role')['name'], ["K12_NSM","LMD_NSM","LSG_NSM","TMD_NSM"])){
				$data['sales_person_code'] = $this->slspn_model->slspn(["filters"=>[
					"nsm_code" => $this->session->userdata('profile')['sales_person_code'],
				]]);
				$data['sales_person_code'] = array_column($data['sales_person_code'],"ma_code");
            }
        }else{
            $slspn_details = $this->users_model->user_list(['sales_person_code'=>$data['slspn_code']]);
            if(in_array($slspn_details['role_name'],["K12_NSM","LMD_NSM","LSG_NSM","TMD_NSM"])){
				$data['sales_person_code'] = $this->slspn_model->slspn(["filters"=>[
					"nsm_code" => $slspn_details['sales_person_code'],
				]]);
				$data['sales_person_code'] = array_column($data['sales_person_code'],"ma_code");
            }elseif(in_array($slspn_details['role_name'],["K12_RSM","LMD_RSM","TMD_RSM","LSG_RSM","TMD_LMD_RSM","DISTRIBUTOR_RSM"])){
				$data['sales_person_code'] = $this->slspn_model->slspn(["filters"=>[
					"rsm_bu" => $slspn_details['slspn_bu'],
					"rsm_code" => $slspn_details['sales_person_code'],
				]]);
				$data['sales_person_code'] = array_column($data['sales_person_code'],"ma_code");
            }else{
                $data['sales_person_code'][] = $data['slspn_code'];
            }
        }
        return $data;
    }
  	...
  	public function get_members(){
		$this->load->model('manage_users/users_model');
		if(in_array($this->session->userdata('user_role')['name'], ['K12_MA','LMD_MA','TMD_MA','TMD_LMD_MA'])){
			$users = [];
		}elseif(in_array($this->session->userdata('user_role')['name'],["K12_RSM","LMD_RSM","TMD_RSM",'TMD_LMD_RSM'])){
			$region = $this->session->userdata('profile')['region'];
			$bu = $this->session->userdata('profile')['slspn_bu'];
			$users = $this->users_model->user_list(['filters'=>['region'=>$region,'slspn_bu'=>$bu]]);
		}elseif(in_array($this->session->userdata('user_role')['name'], ["K12_NSM","LMD_NSM","TMD_NSM",'TMD_LMD_NSM'])){
			$bu = $this->session->userdata('profile')['slspn_bu'];
			$users = $this->users_model->user_list(['filters'=>['slspn_bu'=>$bu]]);
		}else{
			$users = $this->users_model->user_list(['with_slspn'=>true]);
		}
		return $users;
	}
   ...
   public function curl_request( $url , $param = null)
   {
	...
	public function get_cond_operators($cond_operators){
   ...
   public function vd($var)
   {
	...
	public function _send_email($tpl_name,$aData)
	{
		...
		$this->CI->email->from($user, $siteConfig['system_name'].' | NOREPLY');
		...
		$this->CI->email->subject($aData['subject']);
